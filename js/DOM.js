...
class DOM {
    ...
    document.addEventListener("click", (e) => {
      ...
      if (!e.target.closest(".more-dropdown") || e.target.closest(".more-li")) {
        ...
        document
          ...
          .forEach((drop) => drop.classList.add("hide"))
      }
  ...
  #updateCompletas(lista, listObj) {
    ...
    const qtdDone = listObj.tarefas?.filter(
      (task) => task.ativo && task.completa
    ).length
  ...
  #newTask(formObj, idLista) {
    if (idLista) {
      ...
      const listObj = this.controle.listas.find((l) =>
        l.tarefas.some((t) => t.idTarefa === taskObj.idTarefa)
      )
      const tarefasDiv = this.pendentes.querySelector(
        `.lista[id="${idLista}"] .tarefas`
      )
  ...
  #renderTarefa(taskObj) {
    const tarefa = document
      .querySelector("#tarefa-template")
      .content.cloneNode(true)
    ...
    tarefa.querySelector(".prazo").textContent = Utils.formatDate(
      taskObj.prazo
    )
  ...
  #taskInit(tarefa, taskObj) {
    ...
    const listObj = this.controle.listas.find((l) =>
      l.tarefas.some((t) => t.idTarefa === idTarefa)
    )
